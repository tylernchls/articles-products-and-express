...
router.route('/')
  ...
  .get((req, res) => {
    Products.allProducts()
      .then(products => {
        res.render('../product_pages/index', {products})
      })
      .catch((err) => {
        console.error(err);
        res.json(err);
...
router.route('/:id')
  .get((req,res) => {
    Products.getById(Number(req.params.id))
      .then(products => {
        res.render('../product_pages/product', {product: products[0]});
      })
      .catch((err) => {
        console.error(err);
        res.json(err);
...
router.route('/:id/edit')
  .get((req, res) => {
    Products.getById(Number(req.params.id))
      .then(products => {
        res.render('../product_pages/edit', {productId: req.params.id, title: products[0]});
      })
      .catch((err) => {
        console.error(err);
        res.json(err);
